...
export default function BuyData() {
  ...
  const handleBuy = () => {
  if (!number || !data) return alert("Enter number and data");

  alert(`Purchased ${data}GB for ${number}`);

  // Clear fields so user can enter again
  setNumber("");
  setData("");
};

  const handleKeyPress = (digit) => setData(prev=>prev+digit);
  ...
  return (
    <MobileWrapper>
      <View style={styles.container}>
        ...
        <Text style={styles.amountLabel}>Data (GB)</Text>
        <Text style={styles.amountDisplay}>{data || "0"} GB</Text>
