...
export default function ForeignCoin() {
  const router = useRouter();
  const [currency, setCurrency] = useState("");
  const [amount, setAmount] = useState("");

   const handleExchange = () => {
    if (!currency || !amount) return alert("Enter currency and amount");

    alert(`Exchanged ₹${amount} to ${currency}`);
  ...
  return (
    <MobileWrapper>
      <View style={styles.container}>
        ...
        <Text style={styles.title}>Foreign Coin</Text>
